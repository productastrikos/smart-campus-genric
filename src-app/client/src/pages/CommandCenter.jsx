...
// Frontend-only Generic-mode dataset for the "Budget vs Actual" chart.
// The real chart's data (enterprise.budget) comes from the backend
// (/api/enterprise -> data/finance_budget.csv, which genuinely contains
// a "Military Training" row) - per explicit instruction not to touch
// backend/data files, Generic mode uses this separate, hardcoded,
// frontend-only array instead of trying to relabel the real ZMU rows.
// Field names (cost_center/budget_maed/actual_maed) intentionally match
// the real dataset's shape exactly, so the existing <Bars> chart config
// below needs zero changes - only which array is passed in differs.
const GENERIC_BUDGET_DATA = [
  { cost_center: 'Academic Services', budget_maed: 420, actual_maed: 395 },
  { cost_center: 'Campus Operations', budget_maed: 360, actual_maed: 382 },
  { cost_center: 'Student Services', budget_maed: 280, actual_maed: 265 },
  { cost_center: 'IT & Digital Services', budget_maed: 330, actual_maed: 348 },
  { cost_center: 'Facilities Management', budget_maed: 410, actual_maed: 398 },
  { cost_center: 'Security & Safety', budget_maed: 240, actual_maed: 228 },
  { cost_center: 'Administration', budget_maed: 300, actual_maed: 315 },
];

/**
 * GENERIC MODE datasets for Critical Device Health and Agentic AI.
 * ================================================================
 * Frontend-only, following the same established pattern as
 * GENERIC_BUDGET_DATA above (this file's existing convention) rather
 * than introducing a new architecture.
 *
 * These exist because BOTH underlying sources are real ZMU backend data
 * that cannot be relabelled without corrupting it:
 *   - Critical Device Health <- /api/iot/sensors, whose `building`
 *     field joins to the real ZMU building registry (Z01-Z12: "Cadet
 *     Accommodation North", "Command HQ & Admin", "Applied Labs
 *     Centre"...).
 *   - Agentic AI <- /api/overview's aiRecommendations, sourced from
 *     data/ai_recommendations.csv, which genuinely contains cadet /
 *     squadron / Exercise Desert Shield wording (e.g. AI-102).
 * Per instruction, the ZMU sources are left completely untouched and
 * Generic mode gets its own internally-consistent dataset instead.
 *
 * Field shapes match the real API rows exactly, so the existing
 * DataTable columns and the recommendation card markup need no changes.
 */
const GENERIC_CRITICAL_DEVICES = [
  { sensor_id: 'GEN-VAV-014', name: 'VAV Damper · B2-14', building: 'Academic Block', health_pct: 0, status: 'offline' },
  { sensor_id: 'GEN-GLS-021', name: 'Glass-break Sensor · SC-21', building: 'Student Center', health_pct: 0, status: 'offline' },
  { sensor_id: 'GEN-TMP-008', name: 'Temperature Sensor · LIB-08', building: 'Central Library', health_pct: 11, status: 'offline' },
  { sensor_id: 'GEN-PIR-032', name: 'PIR Motion Sensor · ADM-32', building: 'Main Administration', health_pct: 34, status: 'fault' },
  { sensor_id: 'GEN-DAY-011', name: 'Daylight Sensor · RB-11', building: 'Research Building', health_pct: 37, status: 'fault' },
  { sensor_id: 'GEN-FLW-006', name: 'Flow Meter · FAC-06', building: 'Facilities Building', health_pct: 44, status: 'degraded' },
];

export const GENERIC_AI_RECOMMENDATIONS = [
  {
    rec_id: 'AI-201', domain: 'Campus Ops', confidence_pct: 92, status: 'action taken',
    trigger: 'AHU-02 vibration + temp drift (Academic Block)',
    recommendation: 'Auto-created CMMS work order WO-2214; shifted load to AHU-01/03; predicted bearing failure in 6 days',
    impact: 'Avoids unplanned outage of 14 classrooms',
  },
  {
    rec_id: 'AI-202', domain: 'Energy', confidence_pct: 84, status: 'pending approval',
    trigger: 'Dining kWh spike +90% (kitchen exhaust runtime)',
    recommendation: 'Reschedule exhaust purge cycle to 22:00; recommission VFD setpoints',
    impact: 'Est. saving 210 MWh/yr',
  },
  {
    rec_id: 'AI-203', domain: 'Occupancy', confidence_pct: 88, status: 'pending approval',
    trigger: 'Central Library occupancy below 20% after 21:00 across 14 days',
    recommendation: 'Extend HVAC setback window and dim non-egress lighting circuits after 21:00',
    impact: 'Est. saving 96 MWh/yr with no impact on egress lighting compliance',
  },
  {
    rec_id: 'AI-204', domain: 'Security', confidence_pct: 79, status: 'pending approval',
    trigger: 'Repeated after-hours badge reads at Research Building side entrance',
    recommendation: 'Raise door schedule enforcement and route alerts to the on-shift security operator',
    impact: 'Closes an access-control gap flagged on 6 consecutive nights',
  },
  {
    rec_id: 'AI-205', domain: 'Device Health', confidence_pct: 91, status: 'action taken',
    trigger: 'Six environmental sensors below 45% health across four buildings',
    recommendation: 'Batch-scheduled field replacement; degraded nodes excluded from analytics until verified',
    impact: 'Restores environmental telemetry coverage to 99% of monitored zones',
  },
];
...
export default function CommandCenter({ titleKey }) {
  ...
  // Generic-mode-only: the UNSLICED alert feed. /overview caps its list
  // at slice(0,8) server-side, so filtering ZMU rows out of that slice
  // left the panel short. /alerts returns the same records, uncapped.
  ...
  const criticalDevices = isGeneric
    ? GENERIC_CRITICAL_DEVICES
    : (iot?.sensors || []).filter((s) => s.status !== 'online').slice(0, 6);
  const aiRecommendations = isGeneric ? GENERIC_AI_RECOMMENDATIONS : data.aiRecommendations;

  /* Cross-domain alerts: real backend data (data/alerts.csv via
     /api/overview). 9 of the 12 rows are already genuinely generic
     (AHU supply-fan fault, IAM credential-stuffing, library CO2, data
     diode schema drift, dining energy spike, ICT budget, purchase
     requisitions, partner roster sync, CCTV offline). Only 3 are
     ZMU-specific, so Generic Mode FILTERS the real feed rather than
     inventing replacements: nothing fabricated, and no ZMU record is
     ever shown under a generic label.
     Filtered out in Generic Mode:
       - "1 cadets flagged high injury risk (ACWR > 1.4) ahead of field
         exercise" (domain Readiness, source HPO Predictive Analytics)
       - "2 weapons overdue for return > 10h - armoury reconciliation"
       - "Saqr Company avg sleep 5.9h ..." (named ZMU company)
     ZMU Mode uses data.alerts untouched. */
  ...
  /* domainStatus: Generic Mode drops the ZMU "Readiness & Performance"
     card and replaces it with a real IoT device-health card built from
     the /iot/sensors feed this page already fetches (total/online/faults
     - genuine values, not invented), so the row keeps its four cards and
     no gap appears. Cadet-worded sub-labels are blanked. */
  ...
  // Generic mode uses the exact same layout/cards/charts below - only
  // specific text content changes, via inline isGeneric checks, not a
  // separate render path. (A previous version of this file used a
  // separate GenericCampusOverview component with a different KPI/panel
  // taxonomy - explicitly reverted per instruction: "I do NOT want a
  // redesign... I want the existing layout, cards, charts, KPIs,
  // sections to remain exactly as they are now.")

  return (
    <>
      <PageHeader
        title={isGeneric ? 'Campus Overview' : t(titleKey || 'cc.title')}
        subtitle={isGeneric ? 'Unified operational picture across Learning, Availability, Enterprise and Smart Operations — single student ID, governed exchange backbone' : t('cc.subtitle')}
        right={<StatusChip kind="info">{k.systemsOnline}/{k.systemsTotal} {t('cc.systemsOnline')}</StatusChip>}
      />

      <KPIGrid>
        <KPICard label={isGeneric ? 'Students Enrolled' : t('cc.kpi.cadets')} value={fmt.int(k.cadetsEnrolled)} icon={<IcoPeople />} rag="normal"
          subValues={[{ label: t('exec.sub.attendance'), value: `${k.attendanceAvg}%` }, { label: isGeneric ? 'Departments' : t('cc.sub.companies'), value: '4' }]}
          onClick={() => setDetail(isGeneric ? {
            title: 'Students Enrolled', subtitle: `${k.cadetsEnrolled} students enrolled across four departments`, source: 'SIS · Single Student ID',
            stats: [
              ...
              { label: 'Departments', value: 'Falcon · Oryx · Saqr · Ghaf' },
            ],
            content: (
              <>
                <p style={{ fontSize: 12, color: 'var(--app-text-muted)', lineHeight: 1.6, marginBottom: 12 }}>
                  Enrolment is keyed to the single immutable Student ID issued at onboarding by IAM and propagated
                ...
                <Bars data={data.readinessBySquadron} x="squadron" height={200}
                  series={[{ key: 'academic', name: 'Performance (scaled)', color: C.blue }]} hideLegend />
              </>
            ),
          } : {
        ...
        <KPICard label={isGeneric ? 'System Availability' : t('cc.kpi.composite')} value={k.compositeReadiness} unit="/ 100" icon={<IcoTarget />} rag="normal" trend={1.8}
          subValues={[{ label: isGeneric ? 'Sensor availability' : t('cc.sub.wearable'), value: k.wearableReadiness }, { label: t('exec.sub.source'), value: 'SIS + HPO' }]}
          onClick={() => setDetail(isGeneric ? {
            title: 'System Availability', subtitle: 'Weighting: 40% uptime · 25% response time · 25% throughput · 10% error rate', source: 'Ops Telemetry',
            stats: [
              { label: 'Availability', value: `${k.compositeReadiness}`, sub: '/ 100', tone: 'up' },
              { label: 'Response', value: `${k.wearableReadiness}`, sub: 'sensor-derived' },
              { label: 'Uptime', value: `${k.attendanceAvg}%` },
            ],
            content: (
              <>
                <Bars data={data.readinessBySquadron} x="squadron" height={220}
                  series={[
                    ...
                    { key: 'fitness', name: 'Throughput', color: C.violet },
                    { key: 'academic', name: 'Performance', color: C.cyan },
                  ]} />
                <p style={{ fontSize: 11.5, color: 'var(--app-text-faint)', marginTop: 10 }}>
                  Sensor-derived responsiveness currently averages {k.wearableReadiness}/100 across connected systems.
                </p>
              </>
            ),
          } : {
          ...
          onClick={() => setDetail({
            ...
            content: (
              <DataTable
                ...
                rows={activeAlerts.filter((a) => ['critical', 'high'].includes(a.severity))} />
            ),
          })} />
        ...
        <Panel title={t('exec.panel.topRankers')} sub={t('exec.panel.topRankersSub')}>
          <DataTable maxHeight={248}
            columns={[
              ...
              { key: 'name', label: isGeneric ? 'Student' : t('exec.col.cadet') },
              { key: 'company', label: isGeneric ? 'Department' : t('exec.col.company') },
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) minmax(0, 1fr)', gap: 14, marginBottom: 14 }}>
        <Panel
          title={isGeneric ? 'Budget vs Actual Cost by Campus Area' : t('exec.panel.budgetByCostCenter')}
          sub={isGeneric ? 'Spend to date against allocated budget, by campus area' : t('exec.panel.budgetByCostCenterSub')}
        >
          {(isGeneric || enterprise) ? (
            <Bars data={isGeneric ? GENERIC_BUDGET_DATA : enterprise.budget} x="cost_center" layout="vertical" height={240}
              series={isGeneric ? [
                { key: 'budget_maed', name: 'Budget', color: 'var(--app-accent-strong)' },
                { key: 'actual_maed', name: 'Actual', color: 'var(--app-accent)' },
              ] : [
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1.6fr) minmax(0, 1fr)', gap: 14, marginBottom: 14 }}>
        ...
        <Panel title={isGeneric ? 'Department Availability' : t('cc.panel.company')} sub={isGeneric ? 'Comparative performance across campus departments' : t('cc.panel.companySub')}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1.2fr) minmax(0, 1fr)', gap: 14 }}>
        <Panel title={t('cc.panel.agentic')} sub={isGeneric ? 'Autonomous correlation across OT, HVAC, energy, occupancy and security domains' : t('cc.panel.agenticSub')}>
