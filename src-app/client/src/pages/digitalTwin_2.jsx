...
export default function DigitalTwin2() {
  ...
  useEffect(() => {
    ...
    const roster2 = generateRoster({
      ...
      const id = `${isGeneric ? 'C2' : 'ZMU-C2'}-${1000 + i}`;
    ...
    for (const key of ALL_LAYER_KEYS) {
      ...
      // GENERIC MODE: people/cadet markers are never rendered, regardless
      // of the (still-existing, still-toggleable-in-ZMU) 'personnel' layer
      // checkbox state. ZMU mode is completely unaffected — it still just
      // follows visibility[key] exactly as before.
      if (key === 'personnel') personnelLayer.setVisible(!isGeneric && visibility[key]);
      if (key === 'campus2') {
    ...
    const onMouseMove = (e) => {
      ...
      // GENERIC MODE: PersonnelLayer.pickAt() ignores its own setVisible()
      // flag (it raycasts the underlying meshes regardless), so hover
      // picking must be skipped outright here or an invisible-but-still-
      // present marker could still surface a cadet name/ID via
      // PersonnelPopup. `person` stays null the same way it would if the
      // cursor simply weren't over anyone.
      const person = isGeneric ? null : personnelLayer.pickAt(clientX, clientY);
      if (person) {
    ...
    const onClick = (e) => {
      const { clientX, clientY } = e.originalEvent;
      // GENERIC MODE: same reasoning as onMouseMove above — never let a
      // click resolve to a person, so PersonnelPanel (name/ID/role/
      // squadron/HPO-style fields) can never open.
      const person = isGeneric ? null : personnelLayer.pickAt(clientX, clientY);
      if (person) {
  ...
  // GENERIC MODE re-sync: the scene-setup effect above only runs once,
  // when the map/gis/buildings first become ready — it applies the
  // isGeneric value that was true AT THAT MOMENT and never runs again.
  // Switching Application Mode later (in either direction) after the
  // scene has already built must still control the people layer, so this
  // effect exists purely to re-apply personnel visibility whenever
  // isGeneric itself changes, without touching anything else. It's the
  // ONLY thing this effect does — it doesn't affect ZMU-mode's own
  // visibility[key] checkbox state, buildings, cameras, or any other
  // layer at all, and in ZMU mode it just re-applies the same
  // visibility[key] value that was already showing.
  useEffect(() => {
    if (!sceneReady) return;
    personnelLayerRef.current?.setVisible(!isGeneric && visibility.personnel);
    if (isGeneric) {
      // Also drop any currently-open person hover/selection immediately on
      // switching to Generic, rather than waiting for the next mouse move —
      // otherwise a panel opened just before the switch could linger on
      // screen with cadet details until the user moves the cursor.
      personnelLayerRef.current?.setHoveredId(null);
      personnelLayerRef.current?.setSelectedId(null);
      setHoveredPerson(null);
      setSelectedPersonId(null);
      selectedPersonRef.current = null;
  ...
  function toggleLayer(key) {
    setVisibility((prev) => {
      ...
      if (map) {
        ...
        // Same GENERIC-mode rule as the initial-visibility loop above —
        // clicking the (hidden-in-Generic, see LayerControl) checkbox can
        // never turn people markers back on while isGeneric is true.
        if (key === 'personnel') personnelLayerRef.current?.setVisible(!isGeneric && next[key]);
        if (key === 'campus2') {
  ...
  function handleSimulateBreach() {
    ...
    // GENERIC MODE: the breach demo still runs (fence animation only) —
    // it just never puppets a tracked person marker along the path, since
    // people markers are not rendered/picked in Generic at all.
    const intruder = isGeneric ? null : personnelLayerRef.current?.pickRandomActivePerson();
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '100vh', background: '#0a0f16' }}>
      ...
      {/* GENERIC MODE: never render the cadet detail panel/tooltip, even
          defensively — onClick/onMouseMove already keep isGeneric from
          ever setting selectedPersonId/hoveredPerson, this is belt-and-
          braces so no cadet name/ID/role/squadron/HPO field can reach the
          screen in Generic mode under any code path. */}
      {!isGeneric && selectedPersonId && (() => {
      ...
      {!error && !sceneReady && (
        <div style={{
          ...
          <div style={{ fontSize: 11.5, color: 'rgba(188,239,255,0.55)' }}>
            {loading ? (isGeneric ? 'Loading live facility geometry…' : 'Loading live campus geometry from zmu_db…') : 'Building 3-D scene…'}
          </div>
        </div>
      )}
