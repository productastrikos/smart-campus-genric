...
/* Generic-mode role labels. 'cadet' is the only ZMU/military-specific
   role in the roster - every other role (faculty, visitor, security,
   maintenance, medical, administration) is already neutral and is
   deliberately left alone. */
const GENERIC_ROLE_LABELS = { ...ROLE_LABELS, cadet: 'Student' };
/* The generated display name carries a military rank prefix
   ("Officer Cadet Hassan Al Dhaheri"). Generic mode strips just that
   prefix for display - the underlying person record, id, movement,
   telemetry and every other Digital Twin behaviour are untouched. */
const stripRank = (n = '') => n.replace(/^(Officer Cadet|Cadet)\s+/, '');
...
export default function PersonnelPopup({ person, x, y, lonLat }) {
  ...
  return (
    <div style={{
      ...
      <div style={{ fontWeight: 700, marginBottom: 1 }}>{isGeneric ? stripRank(person.name) : person.name}</div>
      <div style={{ color: '#7fd8ff', fontSize: 10 }}>
        {person.id} · {(isGeneric ? GENERIC_ROLE_LABELS : ROLE_LABELS)[person.role] || person.role} · {person.department}
      </div>
