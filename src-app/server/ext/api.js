...
function registerExt(app, express, db) {
  ...
  const USERS = {
    ...
    // Generic-mode demo aliases. ADDITIVE ONLY - every ZMU account above
    // is untouched and keeps working exactly as before. These map to the
    // SAME internal roles ('squadron1'/'squadron2'), so RBAC,
    // ROLE_ROUTES and homeFor() need no changes: the server still
    // returns role 'squadron1' regardless of which username was used to
    // sign in. They exist because the login handler does a strict
    // USERS[username] lookup - without a real entry here, a Generic-mode
    // username of 'departmentlead1' would 401 with "Unknown account".
    departmentlead1: { username: 'departmentlead1', password: 'departmentlead1', role: 'squadron1', name: 'Department Lead 1' },
    departmentlead2: { username: 'departmentlead2', password: 'departmentlead2', role: 'squadron2', name: 'Department Lead 2' },
  };
