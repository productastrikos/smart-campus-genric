...
import { GENERIC_BUILDING_NAME_MAP } from '../../../config/cameras';

/* Same display-only building-name remap as CameraPanel - see the note
   there. Only the `place` segment of the name can carry ZMU wording. */
...
export default function CameraPopup({ camera, x, y }) {
  ...
  return (
    <div style={{
      ...
      <div style={{ color: '#7fd8ff', fontSize: 10, marginTop: 3 }}>{isGeneric ? genericName(camera.name) : camera.name}</div>
      <div style={{ color: 'rgba(220,240,250,0.6)', fontSize: 9.5, marginTop: 2 }}>
        {isGeneric ? genericName(camera.zone) : camera.zone} · {camera.status === 'offline' ? 'no signal' : `${camera.health.fps} fps`}
      </div>
