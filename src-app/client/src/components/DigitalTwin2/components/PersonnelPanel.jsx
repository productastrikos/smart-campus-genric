...
/* Generic-mode overrides - display only. 'cadet' is the sole military
   role; the rest are already neutral. Underlying person records, ids,
   telemetry and Digital Twin behaviour are untouched. */
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div>
            <div style={{ fontSize: 14, fontWeight: 700, color: '#fff' }}>{isGeneric ? stripRank(person.name) : person.name}</div>
            <div style={{ fontSize: 10.5, color: '#7fd8ff' }}>{person.id} · {(isGeneric ? GENERIC_ROLE_LABELS : ROLE_LABELS)[person.role] || person.role}</div>
          </div>
        </div>
