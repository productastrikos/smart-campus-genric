...
/* GENERIC MODE display remaps for Integration & Data.
   All figures (match rates, message counts, latency, error rates, record
   counts) are REAL backend data and pass through untouched - only the
   ZMU/military-specific LABELS are relabelled. Anything already neutral
   (Staff / HR ID, Course Catalogue, Asset Register, Supplier Master,
   SIS/LMS/IAM/BMS/SIEM flows) maps to itself. */
const GENERIC_ENTITY_MAP = {
  'Cadet ID (immutable)': 'Student ID (immutable)',
};
/* Flow + ICD names: "HPO" is the ZMU human-performance unit, "Garmin"
   its issued wearable, "DoF" the UAE Department of Finance, and
   "readiness scores" its military readiness metric. */
const GENERIC_TEXT_SUBS = [
  [/Wearables \/ HPO ↔ analytics — readiness scores to SIS/g, 'Wearables / Wellness ↔ analytics — wellbeing scores to SIS'],
  [/Garmin Health API ↔ HPO middleware/g, 'Wearable Health API ↔ Wellness middleware'],
  [/DoF statutory interface/g, 'Statutory finance interface'],
  [/DoF statutory MFT exchange/g, 'Statutory MFT exchange'],
  [/DoF statutory exchange/g, 'Statutory finance exchange'],
  [/Garmin Health API/g, 'Wearable Health API'],
  [/\bGarmin\b/g, 'Wearable'],
  [/\bHPO\b/g, 'Wellness'],
  [/\bDoF\b/g, 'Finance Authority'],
];
...
export default function Integration() {
  ...
  /* Display-only remaps. Declared AFTER the !data guard, since `data` is
     null on the first render before the API resolves. */
  const gtext = (v) => {
    if (!isGeneric || typeof v !== 'string') return v;
    return GENERIC_TEXT_SUBS.reduce((acc, [re, to]) => acc.replace(re, to), v);
  };
  const flows = isGeneric
    ? (data.flows || []).map((f) => ({ ...f, name: gtext(f.name), systems: gtext(f.systems) }))
    : data.flows;
  const masterData = isGeneric
    ? (data.masterData || []).map((m) => ({ ...m, entity: GENERIC_ENTITY_MAP[m.entity] || m.entity }))
    : data.masterData;
  const icds = isGeneric
    ? (data.icds || []).map((i) => ({ ...i, interface: gtext(i.interface) }))
    : data.icds;
  ...
  return (
    <>
      <PageHeader
        ...
        subtitle={`Governed exchange backbone — API gateway & ESB/iPaaS · master data & single ${isGeneric ? 'student' : 'cadet'} ID · controlled file-exchange gateway · data lake & analytics`}
      ...
      <KPIGrid min={185}>
        ...
        <KPICard label={isGeneric ? "Student ID Match" : "Cadet ID Match"} value={`${k.cadetIdMatch}%`} icon={<IcoDatabase />} rag="normal"
          subValues={[{ label: 'Golden records', value: '2,096' }]}
          onClick={() => setDetail({
            title: isGeneric ? 'Student ID Match' : 'Cadet ID Match', subtitle: `${k.cadetIdMatch}% match rate on the single immutable ${isGeneric ? 'Student' : 'Cadet'} ID`, source: 'Master Data Management',
            content: (
              <DataTable
                columns={[
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1.5fr) minmax(0, 1fr)', gap: 14, marginBottom: 14 }}>
        ...
        <Panel title="Master Data — Golden Records" sub="Single identifier convention across all systems">
          ...
          <div style={{ fontSize: 10.5, color: 'var(--app-text-faint)', marginTop: 10, lineHeight: 1.5 }}>
            {isGeneric
              ? 'Single immutable Student ID issued by IAM at enrolment; propagated by joiner-mover-leaver provisioning to SIS, LMS, Wellness, Asset Management and physical access (flow 3).'
              : 'Single immutable Cadet ID issued by IAM at enrolment; propagated by joiner-mover-leaver provisioning to SIS, LMS, HPO, WMS and physical access (flow 3).'}
          </div>
        </Panel>
      </div>
