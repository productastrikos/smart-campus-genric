...
/* GENERIC MODE clip pool. The ZMU pool above (/v4, /v5, /v9) is real
   military training footage - soldiers, weapons, perimeter assault -
   which must never appear in Generic Mode. These are the user-supplied
   generic campus clips in client/public/videos/.
   With ~180 cameras and 11 clips, repetition is unavoidable - but each
   camera picks deterministically by hash and seeks to its own
   feedOffset, so no two cameras open at the same frame even when they
   share a clip. The pool is listed smallest-file-first so the most
...
const GENERIC_LOCAL_CLIPS = [
  ...
  '/videos/generic/v5.mp4',    // 7.1 MB
  '/videos/generic/v2.mp4',    // 19 MB
  '/videos/building/b3.mp4',   // 29 MB
  '/videos/generic/v6.mp4',    // 38 MB
...
/* The clip pool is chosen at record-build time. makeCameraRecord() is a
   plain function (not a React component), so it can't use the
   useAppMode() hook - it reads the same localStorage key the
   AppModeProvider persists to. That key is the single source of truth
   for the mode, so the two can't disagree. */
function isGenericMode() {
  try {
    return localStorage.getItem('zmu_app_mode') === 'generic';
  } catch {
    return false;
  }
}
...
export function makeCameraRecord({ campus, seq, kind, place, lonLat }) {
  ...
  return {
    ...
    videoUrl: status === 'offline' || !useLocalClip ? '' : (() => {
      const pool = isGenericMode() ? GENERIC_LOCAL_CLIPS : LOCAL_CLIPS;
      return pool[feedHash % pool.length];
    })(),
    // Generic Mode never uses the ScreenPal embeds - those are the ZMU
    // demo feeds - so it always falls through to the local generic pool.
    embedUrl: status === 'offline' || useLocalClip || !EMBEDS.length || isGenericMode() ? '' : EMBEDS[feedHash % EMBEDS.length],
