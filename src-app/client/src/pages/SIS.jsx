...
/* Generic-mode display remap. Same convention as pages/Academic.jsx: only
   the ZMU/military-specific WORDING changes (institution name, "cadet",
   the "Military" stream label) — every real value, id, key and record is
   passed straight through from the backend untouched. */
const GENERIC_INSTITUTION = 'the university';
const GENERIC_STREAM_LABEL = { Campus: 'Training' };
const stream = (isGeneric, label) => (isGeneric && GENERIC_STREAM_LABEL[label]) || label;
...
function StudentProfile({ studentId, onGrade }) {
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1fr) minmax(0,1.6fr)', gap: 14, alignItems: 'start' }}>
      <Panel title={s.name} sub={`${s.username} · ${s.company} ${isGeneric ? 'Group' : 'Company'} · Year ${s.year} · ${s.programme || '—'}`}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
          {[['College', s.college_label || s.college], ['GPA', s.gpa ?? '—'], ['Academic %', s.academic_pct], ['Composite', s.composite],
            ['Fitness', s.fitness?.score], [stream(isGeneric, 'Military'), s.military?.score], ['Conduct', s.conduct?.score],
        ...
        <div style={{ marginTop: 12 }}>
          <Advisory items={[
            ...
            `${s.name.split(' ')[0]} reads on one immutable ${isGeneric ? 'Student' : 'Cadet'} ID across SIS, LMS, HPO, ${isGeneric ? 'training' : 'military'} and conduct — every figure on this page is joined, not re-keyed.`,
            s.fitness?.score != null ? `Streams — fitness ${s.fitness.score}, ${isGeneric ? 'training' : 'military'} ${s.military?.score}, conduct ${s.conduct?.score}; the weakest one is where an intervention returns the most composite.` : 'Stream scores sync nightly into the composite.',
          ]} />
        </div>
      </Panel>
  ...
  return (
    <>
      <PageHeader title="Student Information System"
        subtitle={isGeneric
          ? `Unified SIS/ERP across the four academic colleges of ${GENERIC_INSTITUTION} · one governed student record · Term Fall 2026 (202610)`
          : `Unified SIS/ERP across the four academic colleges of Zayed Military University · one governed cadet record · Term Fall 2026 (202610)`}
      ...
      {tab === 'Institution Dashboard' && (
        <>
          <KPIGrid>
            <KPICard label={isGeneric ? 'Total Students' : 'Total Cadets'} value={k.students} icon={<IcoPeople />} subValues={[{ label: 'Colleges', value: 4 }]} />
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1fr) minmax(0,1fr)', gap: 14, marginBottom: 14 }}>
            <Panel title="Enrolment by College" sub={isGeneric ? `Four academic colleges of ${GENERIC_INSTITUTION}` : 'Four academic colleges of Zayed Military University'}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1.4fr) minmax(0,1fr)', gap: 14, marginBottom: 14 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <Advisory items={[
                data.atRisk.length ? `${data.atRisk.length} ${isGeneric ? 'students' : 'cadets'} flagged at-risk on fitness/conduct — draft intervention briefs are ready in the Order of Merit view.` : `No at-risk ${isGeneric ? 'students' : 'cadets'} in scope this term.`,
          ...
          <Panel title="Holds & At-Risk" sub="Registration blocks originate in the Conduct register (score < 65) and release automatically">
            <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1fr) minmax(0,1fr)', gap: 14 }}>
              ...
              <DataTable maxHeight={220}
                columns={[
                  { key: 'name', label: isGeneric ? 'At-risk student' : 'At-risk cadet' }, { key: 'tenant', label: 'College' },
      ...
      {tab === 'Holds · SOAHOLD' && (
        <Panel title="SOAHOLD — Registration Holds" sub="Automatic conduct holds (score < 65) block new registration with 409 · merits release them">
          {data.holds.length === 0 ? <div style={{ fontSize: 12.5, color: 'var(--app-text-faint)', padding: 8 }}>No active registration holds in scope.</div> : (
            <DataTable
              columns={[
                { key: 'student_id', label: 'ID' }, { key: 'name', label: 'Student' }, { key: 'company', label: isGeneric ? 'Group' : 'Company' },
