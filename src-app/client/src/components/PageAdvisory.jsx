...
const POOLS = {
  ...
  /* Sustainability & Energy — Generic-mode-only core module.
     ADDITIVE: no existing pool above is touched. This key is unreachable in
     ZMU mode because the /sustainability route is never registered there.

     Without this entry the route would fall through to DEFAULT_KEY ('/'),
     whose pool contains cohort/readiness wording — so this exists specifically
     to keep the Generic module's advisory strip neutral.

     These lines describe the SCOPE AND LIMITS of the dataset rather than
     asserting any metric, so nothing here can drift from the real figures the
     page renders. `ar` is intentionally omitted; poolFor() already falls back
     to `en`, and the language toggle is hidden in Generic mode. */
  '/sustainability': {
    en: [
      'Every figure on this page is aggregated from the recorded daily building series — energy, water and on-site generation only.',
      'The reporting window is read from the data itself; no period beyond the recorded range is shown or projected.',
      'On-site generation is recorded at a subset of buildings, so generation totals are reported against that subset rather than campus-wide.',
      'Consumption and generation are held as independent series, so a net grid-import figure is not derived from them.',
      'The carbon figure is an estimate produced by applying a stated grid emission factor to metered consumption — it is not measured emissions data.',
      'Energy intensity is period-cumulative against recorded floor area and is not annualised, as the dataset does not span a full year.',
      'No targets, benchmarks or savings baselines exist in the source data, so none are presented alongside these metrics.',
    ],
  },
  // Smart Classrooms (room-level Digital Twin — UIIC). Ported from the
  // module's own advisory strip so this stays the single per-page surface.
  '/smart-classrooms': {
    en: [
      'D-GF-03: 4 BLE cards active without a presence increment — possible proxy attendance, flag for integrity review.',
      'D-GF-03: Instructor is off-site — auto-hybrid bridging engaged for CSC 340 on the IdeaHub, no manual start needed.',
      'D-GF-07: HVAC actuator fault — CO₂ is trending above the comfort threshold; raise a work order before the next session.',
      'IDF-01: PoE budget at 78% utilisation across the 8-port switch — headroom is thin for another AV endpoint.',
      'D-FF-02: Room vacant 45+ minutes — energy setback mode is recommended and can be applied from the room twin.',
      'Oracle PeopleSoft roster sync is healthy; the audit hash was verified at 09:58 GST.',
      'BLE tag drift detected near the D-GF-05 corridor — recalibration suggested to keep occupancy counts trustworthy.',
    ],
    ar: [
      'D-GF-03: أربع بطاقات BLE نشطة دون زيادة في الحضور — احتمال حضور بالوكالة، يُرفع للمراجعة.',
      'D-GF-03: المحاضر خارج الموقع — تم تفعيل الجسر الهجين تلقائيًا لمقرر CSC 340 دون تدخل يدوي.',
      'D-GF-07: عطل في مشغّل التكييف — ثاني أكسيد الكربون يتجاوز حد الراحة؛ يُنصح بإصدار أمر عمل.',
      'IDF-01: استهلاك ميزانية PoE عند ٧٨٪ على المحوّل ذي المنافذ الثمانية — الهامش محدود لأي طرف صوتي/مرئي إضافي.',
      'D-FF-02: القاعة شاغرة أكثر من ٤٥ دقيقة — يُنصح بتفعيل وضع توفير الطاقة من التوأم الرقمي للقاعة.',
      'مزامنة كشوف Oracle PeopleSoft سليمة؛ تم التحقق من بصمة التدقيق عند ٠٩:٥٨ بتوقيت الخليج.',
      'انحراف في وسوم BLE قرب ممر D-GF-05 — يُقترح إعادة المعايرة للحفاظ على دقة عدّ الإشغال.',
    ],
  },
};
