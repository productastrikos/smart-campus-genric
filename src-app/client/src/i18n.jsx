...
const STR = {
  ...
  'page.smartClassrooms': { en: 'Smart Classrooms', ar: 'الفصول الذكية' },
...
export function LangProvider({ children }) {
  ...
  /* Generic Mode is English-only. The LangToggle button is hidden there,
     but the stored `lang` persists in localStorage - so switching to
     Arabic in ZMU and then flipping to Generic previously left the whole
     app in Arabic with no visible way back. Forcing the effective
     language here covers every t() call in the app at once, and does NOT
     overwrite the stored preference: switching back to ZMU restores the
     user's Arabic selection exactly as they left it. */
  const effectiveLang = isGeneric ? 'en' : lang;
  ...
  useEffect(() => {
    ...
    // Persist the user's real choice, not the mode-forced value.
    localStorage.setItem('zmu_lang', lang);
  }, [lang, effectiveLang]);
  const t = (key) => {
    const s = STR[key];
    if (!s) return key;
    return s[effectiveLang] || s.en || key;
  };
  const toggle = () => setLang((l) => (l === 'en' ? 'ar' : 'en'));
  return <LangCtx.Provider value={{ lang: effectiveLang, dir, t, toggle }}>{children}</LangCtx.Provider>;
}
