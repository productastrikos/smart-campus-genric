  ...
  return (
    <>
      <PageHeader
        ...
        subtitle={isGeneric ? 'BMS supervisory layer · EMS analytics · CCTV & access control · IoT across 9 zones — vendor-neutral overlay on base-build systems' : (ar ? 'طبقة إشراف نظام إدارة المباني · تحليلات الطاقة · المراقبة والتحكّم بالدخول · إنترنت الأشياء عبر ٩ مناطق' : 'BMS supervisory layer · EMS analytics · CCTV & access control · WMS · IoT across 9 zones — vendor-neutral overlay on base-build systems')}
      ...
      <KPIGrid min={190}>
        ...
        <KPICard label={ar ? 'كاميرات المراقبة المتصلة' : 'CCTV Online'} value={`${k.camerasOnline}/${k.camerasTotal}`} icon={<IcoCamera />}
          ...
          onClick={() => setDetail({
            ...
            stats: [
              ...
              ...(isGeneric
                ? [{ label: 'Parking occupancy', value: `${k.parkingOccupancy}%` }]
        ...
        {isGeneric ? (
          /* Generic replacement for the ZMU "Weapons Issued Out" KPI.
             Powered by REAL data: data.assets (from /api/campus, built
             from bms_assets.csv) - actual MEP equipment in fault or
             degraded state, with genuine health_pct values. The KPI slot
             stays filled so the grid doesn't gap, and the View Details
             panel is fully generic too (no weapon/armoury/cadet terms). */
          <KPICard label="Equipment Needing Attention" value={k.assetsInFault + k.assetsDegraded} icon={<IcoWrench />}
            rag={k.assetsInFault > 0 ? 'critical' : k.assetsDegraded > 0 ? 'warning' : 'normal'}
            subValues={[{ label: 'In fault', value: k.assetsInFault }, { label: 'Degraded', value: k.assetsDegraded }]}
            onClick={() => setDetail({
              title: 'Equipment Needing Attention',
              subtitle: `${k.assetsInFault} in fault · ${k.assetsDegraded} degraded`,
              source: 'Building Management System — Asset Registry',
              content: (
                <DataTable maxHeight={280}
                  columns={[
                    { key: 'asset_id', label: 'Asset' },
                    ...
                    { key: 'health_pct', label: 'Health', align: 'right', render: (v) => `${v}%` },
                    { key: 'status', label: 'Status', render: (v) => <StatusChip kind={sevChip(v)}>{v.toUpperCase()}</StatusChip> },
                  ]}
                  rows={data.assets} />
              ),
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1.4fr) minmax(0, 1fr)', gap: 14 }}>
        {isGeneric ? (
          /* Generic replacement for the ZMU Weapon Management System
             panel. Real data: data.assets from /api/campus (bms_assets
             .csv) - genuine MEP equipment with health_pct, runtime and
             next_maintenance. Fills the same grid column so the layout
             doesn't gap, with no weapon/armoury/cadet/unit terminology. */
          <Panel title="Building Equipment Health" sub="MEP assets in fault or degraded state · CMMS work orders auto-raised"
            right={k.assetsInFault > 0 ? <StatusChip kind="danger">{k.assetsInFault} FAULT</StatusChip> : <StatusChip kind="success">NOMINAL</StatusChip>}>
            <DataTable maxHeight={260}
              columns={[
                { key: 'asset_id', label: 'Asset' },
                { key: 'type', label: 'Type' },
                { key: 'building_id', label: 'Building' },
                { key: 'health_pct', label: 'Health', align: 'right', render: (v) => <b style={{ color: v < 40 ? 'var(--app-danger)' : 'var(--app-warning)' }}>{v}%</b> },
                { key: 'next_maintenance', label: 'Next Service' },
                { key: 'status', label: 'Status', render: (v) => <StatusChip kind={sevChip(v)}>{v.toUpperCase()}</StatusChip> },
              ]}
              rows={data.assets} />
          </Panel>
