...
export default function IncidentManagement() {
  ...
  const camName = (id, fallback) =>
  (isGeneric && GENERIC_CAMERA_NAME_MAP[id]) || fallback;

const bldName = (buildingId, fallbackName) => {
  if (!isGeneric) return fallbackName;

  return GENERIC_BUILDING_NAME_MAP[buildingId] || fallbackName;
};
...
const activeIncidents = isGeneric
  ? data.incidents.map((i) => ({
      ...
      // IMPORTANT:
      // Generic building name is selected by REAL building_id,
      // not by the backend building_name string.
      building_name: bldName(i.building_id, i.building_name),
    }))
  : data.incidents;
...

  {
    ...
    render: (v, r) => {
      const displayName = isGeneric
        ? (GENERIC_BUILDING_NAME_MAP[r.building_id] || 'Campus Facility')
        : v;

      return r.building_id ? (
        <Link
          to={`/digital-twin?building=${r.building_id}`}
          style={{
            color: '#22d3ee',
            textDecoration: 'none',
          }}
