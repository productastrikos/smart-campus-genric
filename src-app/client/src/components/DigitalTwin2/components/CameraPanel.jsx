...
/* Generic-mode display remap. The DT camera record's own fields are
   already neutral (kind labels: Main Gate / Perimeter / Parking / Road
   Junction / Building / Ground; model: PTZ Dome 4K etc.), so the ONLY
   ZMU-specific text that can reach this popup is the `place` segment of
   camera.name, which comes from the real GIS building display_name
   (e.g. "Building — Cadet Accommodation North"). That segment is
   remapped using the SAME building map Incident Management uses, so the
   two stay consistent. Display only - the camera record, its id, its
   geometry, telemetry and every DT behaviour are untouched. */
function genericName(name = '') {
  let out = name;
  for (const [zmu, gen] of Object.entries(GENERIC_BUILDING_NAME_MAP)) {
    if (out.includes(zmu)) out = out.split(zmu).join(gen);
  }
  return out;
}
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10, padding: '11px 13px 9px' }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ fontSize: 11, color: '#7fd8ff', marginTop: 3, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
            {isGeneric ? genericName(camera.name) : camera.name}
          </div>
          <div style={{ fontSize: 9.5, color: 'rgba(220,240,250,0.5)', marginTop: 1 }}>
            {isGeneric ? genericName(camera.zone) : camera.zone} · {camera.model}
          </div>
        </div>
        <button
          onClick={onClose}
