...
const WELCOME = {
  generic: "Hi, I'm S!A — the Smart Digital Campus assistant. Ask me about attendance, availability, energy, security or the digital twin.",
  erp: "Hi, I'm the S!A ERP Assistant. Ask me about budget, procurement, headcount, attrition, cash flow or the DoF interface.",
};
...
function ChatBody({ mode, height }) {
  ...
  const questions = suggestedQuestions(mode, isGeneric);
  ...
  function send(text) {
    ...
    // Show the question immediately, then let S!A "think" for 1–2s before replying.
    setMessages((m) => [...m, { role: 'user', text: trimmed }]);
    setInput('');
    setThinking(true);
    const delay = 1000 + Math.random() * 900;
    timerRef.current = setTimeout(() => {
      const reply = askSia(mode, trimmed, ctx, isGeneric);
  ...
  return (
    <div style={{ background: 'var(--app-advisory-panel)', padding: '13px 16px', flexShrink: 0 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        <div>
          <div style={{ fontSize: 13, fontWeight: 700, color: '#fef9ef' }}>{mode === 'erp' ? 'S!A · ERP Assistant' : 'S!A · Campus Assistant'}</div>
...
export default function SiaAgent() {
  ...
  return (
    <div ref={rootRef}>
      ...
      <button
        onClick={() => setOpen((o) => !o)}
        aria-label="Open S!A assistant"
        title="S!A Campus Assistant"
        style={{
