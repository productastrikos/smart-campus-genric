/**
 * S!A Agent — fixed-response knowledge base (no external LLM call).
...
const genericEntries = [
  {
    ...
    answer: () =>
      "I'm S!A — the Smart Digital Campus assistant. Ask me about cadet attendance, readiness scores, campus energy, security alerts, budget, or any of the four domains (Learning & Academic, Military Readiness, Enterprise & Data, Campus & Smart Ops).",
  },
  {
...
const erpEntries = [
  {
    ...
    question: 'What can you help me with?',
    answer: () =>
      "I'm the S!A ERP Assistant. Ask me about budget utilization, procurement status, headcount, attrition, cash flow, or the DoF statutory interface.",
  },
  {
...
/* GENERIC MODE terminology remap.
   The knowledge base is written in ZMU terms (cadets, Garmin/HPO wearables,
   ACWR, armoury/WMS, Military Readiness). Generic Mode is a Smart Digital
   Campus, so the SAME real answers are relabelled on the way out rather than
   maintaining a duplicate knowledge base that could drift out of sync.
   Only wording changes - every figure still comes from the same live ctx. */
const GENERIC_SUBS = [
  [/Military Readiness/g, 'Campus Operations'],
  [/military score/g, 'operations score'],
  [/Weapon Management System \(WMS\)/g, 'Asset Management System'],
  [/\bWMS\b/g, 'Asset Management'],
  [/Z09 armoury/g, 'central stores'],
  [/\barmoury\b/gi, 'stores'],
  [/Weapons outstanding/g, 'Assets outstanding'],
  [/\bweapons?\b/gi, 'assets'],
  [/Garmin Health API/g, 'wearable health API'],
  [/Garmin wearable/gi, 'wearable'],
  [/Garmin devices/g, 'wearable devices'],
  [/\bGarmin\b/g, 'wearable'],
  [/the HPO platform/g, 'the wellness platform'],
  [/\bHPO\b/g, 'Wellness'],
  [/an acute:chronic workload ratio \(ACWR\)/g, 'a sustained-load ratio'],
  [/acute:chronic workload ratio \(ACWR\)/g, 'sustained-load ratio'],
  [/\bACWR\b/g, 'sustained-load ratio'],
  [/injury-risk/g, 'wellbeing-risk'],
  [/cadet attendance/gi, 'student attendance'],
  [/cadet readiness/gi, 'campus readiness'],
  [/Every cadet is issued/g, 'Participating staff are issued'],
  [/\bcadets\b/gi, 'students'],
  [/\bcadet\b/gi, 'student'],
  [/\breadiness\b/gi, 'availability'],
];
function toGeneric(text) {
  if (typeof text !== 'string') return text;
  return GENERIC_SUBS.reduce((acc, [re, to]) => acc.replace(re, to), text);
}

export function askSia(mode, text, ctx, isGeneric = false) {
  const entries = mode === 'erp' ? erpEntries : genericEntries;
  const out = (t) => (isGeneric ? toGeneric(t) : t);
  const hit = match(entries, text);
  if (hit) return out(hit.answer(ctx || {}));
  ...
  return out(prefix + pick.answer(ctx || {}));
}

export function suggestedQuestions(mode, isGeneric = false) {
  const entries = mode === 'erp' ? erpEntries : genericEntries;
  return entries.filter((e) => !e.keywords.includes('hello')).slice(0, 6)
    .map((e) => (isGeneric ? toGeneric(e.question) : e.question));
}
