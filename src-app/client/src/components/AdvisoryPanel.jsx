...
import { GENERIC_AI_RECOMMENDATIONS } from '../pages/CommandCenter';
 ...
 */
// Seed advisories that always lead the panel — course-level actions
// leadership asked to surface first. These are shown before the telemetry-
// derived advisories so they're the first thing seen on opening the panel.
//
// Two parallel arrays, same shape/order, swapped by isGeneric (Step: no
// military wording should ever reach the screen in Generic Mode) — the
// SYE204/ECE210 items have nothing military-specific and are identical in
// both; only GEO140 differs.
...
const GENERIC_SEED_ADVISORIES = [
  SEED_ADVISORIES[0],
  {
    sev: 'act now',
    code: 'GEO140',
    title: 'GEO140 · Field Geography — training vehicle unserviceable',
    body: 'The field training vehicle assigned to today\'s GEO140 field module has broken down. Recommend either rescheduling the class to the next available field slot, or dispatching a backup vehicle from the transport pool so the exercise proceeds.',
  },
  {
    sev: 'act now',
    code: 'ECE210',
    title: 'ECE210 · Instructor absent — Prof. Dr. N. Farouk unavailable',
    body: 'Prof. Dr. N. Farouk is marked absent for today\'s ECE210 session. Recommend assigning a qualified substitute from the same subject area (Electronics) to cover the class so students are not left without instruction.',
  },
];
...
export default function AdvisoryPanel({ open, onClose }) {
  ...
  const seedAdvisories = isGeneric ? GENERIC_SEED_ADVISORIES : SEED_ADVISORIES;
  ...
  const derived = k ? [
    ...
    },
    {
      ...
      body: isGeneric
        ? 'One cohort group\'s sleep is trending low; consider adjusting the evening study/rest schedule before the next assessment.'
        : 'Saqr Company sleep is trending low; consider adjusting the night-training rotation before the next assessment.',
    },
    {
      sev: 'insight',
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: 14 }}>
          ...
          {!data ? (
            ...
            <>
              ...
              {(isGeneric ? GENERIC_AI_RECOMMENDATIONS : data.aiRecommendations).map((r) => (
