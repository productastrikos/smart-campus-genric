...
export function Bars({ data, x, series, height = 220, stacked = false, layout = 'horizontal', hideLegend = false, catWidth = 132, angledLabels = false }) {
  const vertical = layout === 'vertical';
  const multi = series.length > 1;
  /* angledLabels: opt-in for category charts with many long names, where
     the default interval={0} straight labels collide (e.g. 8-12 building
     names). Rotates the labels -35 degrees, anchors them at the end, and
     reserves extra axis height + chart bottom margin so nothing renders
     outside the card. Off by default so every existing chart keeps its
     current appearance exactly. */
  const xAxisHeight = angledLabels ? 68 : 24;
  const bottomMargin = angledLabels ? 12 : 4;
  return (
    <ResponsiveContainer width="100%" height={height}>
      <BarChart data={data} layout={layout} margin={{ top: 8, right: 14, bottom: vertical ? 4 : bottomMargin, left: vertical ? 4 : 4 }} barCategoryGap={vertical ? '18%' : '22%'}>
        ...
        {vertical ? (
          ...
          <>
            <XAxis dataKey={x} tick={angledLabels ? { ...axisStyle, fontSize: 10 } : axisStyle} tickLine={false} axisLine={{ stroke: 'var(--app-chart-grid)' }}
              interval={0} minTickGap={4} tickMargin={angledLabels ? 10 : 8}
              tickFormatter={(v) => truncate(v, angledLabels ? 14 : 12)}
              angle={angledLabels ? -35 : 0} textAnchor={angledLabels ? 'end' : 'middle'}
              height={xAxisHeight} />
